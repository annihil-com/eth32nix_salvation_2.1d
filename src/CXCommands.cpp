...
void xcmd_Namesteal()
{
	...
	Syscall.UI_ExecuteText("ETH32nix_2.1d_set namesteal on; wait 100; ETH32nix_2.1d_set namesteal off\n");
...
void xcmd_unbanCommand()
{
	...
	Syscall.UI_ExecuteText("ETH32nix_2.1d_randguid; ETH32nix_2.1d_randmac; wait 50; ETH32nix_2.1d_randname; reconnect\n");
}


void xcmd_ImmoSpam()
{	// sol: added simple spam, feel free to modify however you like
	Syscall.UI_ExecuteText("say \"^7.:.^0ETH^732^3nix^7.:. ^dSalvation ^n2.1d\n\"\n");
	Syscall.UI_ExecuteText("say \"^7Only ^7LINUX\n\"\n");
}
...
void CXCommands::RegisterCommands()
{
	...
	orig_Cmd_AddCommand("ETH32nix_2.1d_vec", &loadCustomHitbox);
	orig_Cmd_AddCommand("ETH32nix_2.1d_home", &goHome);
	orig_Cmd_AddCommand("ETH32nix_2.1d_immospam", &xcmd_ImmoSpam);
	orig_Cmd_AddCommand("ETH32nix_2.1d_namesteal", &xcmd_Namesteal);
	orig_Cmd_AddCommand("ETH32nix_2.1d_randname", &xcmd_RandName);
	orig_Cmd_AddCommand("ETH32nix_2.1d_cvar", &xcmd_CvarForce);
	orig_Cmd_AddCommand(va("%s",eth32.settings.cvarPrefix), &xcmd_Settings);
	orig_Cmd_AddCommand("ETH32nix_2.1d_toggle", &xcmd_Toggle);
	orig_Cmd_AddCommand("ETH32nix_2.1d_unban", &xcmd_unbanCommand);
	orig_Cmd_AddCommand("ETH32nix_2.1d_randguid", &xcmd_RandGuid);
	orig_Cmd_AddCommand("ETH32nix_2.1d_randmac", &xcmd_RandMac);
	orig_Cmd_AddCommand("ETH32nix_2.1d_load", &xcmd_loadsettings);
	orig_Cmd_AddCommand("ETH32nix_2.1d_save", &xcmd_savesettings);

	// spamz
	orig_Cmd_AddCommand("ETH32nix_2.1d_killspam", &xcmd_killspam);
	orig_Cmd_AddCommand("ETH32nix_2.1d_selfkillspam", &xcmd_selfkillspam);
	orig_Cmd_AddCommand("ETH32nix_2.1d_suicidespam", &xcmd_suicidespam);
	orig_Cmd_AddCommand("ETH32nix_2.1d_deathspam", &xcmd_deathspam);
	orig_Cmd_AddCommand("ETH32nix_2.1d_multikillspam", &xcmd_multikillspam);

	orig_Cmd_AddCommand("ETH32nix_2.1d_specstats", &xcmd_specstats);

	orig_Cmd_AddCommand("+aim", &xcmd_AimKeyDown);
	orig_Cmd_AddCommand("-aim", &xcmd_AimKeyUp);
	orig_Cmd_AddCommand("ETH32nix_2.1d_etproos", &setEtproOS);
