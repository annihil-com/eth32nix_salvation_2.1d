...
bool CSettings::SaveUserSettings(const char *filename)
{
	...
	for (int i=0 ; i<numSettings ; i++)
	{
		...
		fprintf(fp, "ETH32nix_2.1d_set %s %s\n", setting->name, value);
	}
	fclose(fp);
	return true;
}
...
void CSettings::ConsoleCommand(void)
{
	...
	if (Syscall.UI_Argc() == 1){
		Syscall.UI_Print("^dSyntax:  ^o/ETH32nix_2.1d_set ^2[^9setting^9] ^2[^9value^2]\n\n");
		PrintAll();
		return;
	}
...
void CSettings::Toggle(void)
{
	...
	if (Syscall.UI_Argc() == 1){
		Syscall.UI_Print("^dSyntax:  ^o/ETH32nix_2.1d_toggle ^2[^9setting^9]\n");
		return;
	}
